...
## Example
```PHP
// You can use the global elexBeBWPFluent() function
$user = elexBeBWPFluent()->table('users')->find(1);
...
// Or, simply use the global elexBeBWPFluent() function.
// It handles all the ncessary initial setup.
elexBeBWPFluent()->table('my_table')->where('name', '=', 'admin');
```
